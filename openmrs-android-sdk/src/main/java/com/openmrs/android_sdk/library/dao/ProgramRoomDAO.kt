...
interface ProgramRoomDAO {
    ...
    /**
     * Deletes every cached program. `id` (not `uuid`) is this table's primary key and is always
     * freshly autogenerated on insert, so [insertOrUpdatePrograms]'s REPLACE conflict strategy
     * never actually finds a conflicting row - refreshing the cache without this first would just
     * keep appending duplicate rows per uuid instead of replacing them.
     *
